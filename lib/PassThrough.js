...
class SimplexPair extends Stream {
	// Header support
	writeHead(...args) {
	...
	pipeHeaders(...args) {
		return this.readableSide.pipeHeaders(...args);
	}
	pipeMessage(...args) {
	...
	on(event, listener) {
		switch (event) {
			case 'close':
			case 'error':
				// FIXME passthrough.on('error') and readableSide.on('error')
				// don't end up working the same
...
class ResponsePassThrough extends SimplexPair {
	...
	constructor(options) {
		super();
		const self = this;
		// this.unattachedError = new Error('"error" event not attached');
		// process.nextTick(function(){
		// 	if(self.readableSide.listenerCount("error")===0) throw self.unattachedError;
		// });
		this.readableSide = new ReadableSide(options);
		this.writableSide = new WritableSide(options);
